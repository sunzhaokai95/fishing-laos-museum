...
export default function AnglerAssessment() {
  ...
  if (phase === 'result') { const result = ANGLER_RESULTS[resultKey]; return <section className="angler-tool-stage"><small>YOUR WATERSIDE TENDENCY / 水边倾向</small><h2>{result.title}</h2><p>{result.text}</p><div className="mt-7 grid gap-3">{Object.entries(DIMENSIONS).map(([key, label]) => { const value = scores[key] * 25; return <div key={key} className="grid grid-cols-[52px_1fr_34px] items-center gap-3 text-xs"><span>{label}</span><span role="progressbar" aria-label={`${label}维度`} aria-valuemin="0" aria-valuemax="100" aria-valuenow={value} className="h-1.5 bg-zinc-200"><motion.i initial={{ width: 0 }} animate={{ width: `${value}%` }} className="block h-full bg-zinc-800" /></span><span className="font-mono text-[9px] text-zinc-500">{value}</span></div> })}</div><button type="button" onClick={reset} aria-label="重新测试" className="angler-primary-action"><RefreshCcw size={13} aria-hidden="true" />重新测试</button></section> }
  const question = ANGLER_QUESTIONS[step]
  return <section className="angler-tool-stage"><div className="flex justify-between font-mono text-[9px] text-zinc-400"><span>QUESTION {step + 1} / {ANGLER_QUESTIONS.length}</span><span>{Math.round(step / ANGLER_QUESTIONS.length * 100)}%</span></div><h2>{question.question}</h2><div className="mt-7 grid gap-px bg-zinc-200">{question.options.map((option) => <button type="button" key={option.text} onClick={() => answer(option.type)} className="min-h-14 bg-white px-4 py-3 text-left text-sm leading-6 text-zinc-700 hover:bg-zinc-50">{option.text}</button>)}</div></section>
}
