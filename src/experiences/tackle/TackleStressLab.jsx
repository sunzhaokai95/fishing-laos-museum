...
export default function TackleStressLab({ records, onOpen }) {
  ...
  return (
    <section className="border-y border-stone-300" aria-label="钓具受力拆解台">
      <div className="grid min-h-[610px] lg:grid-cols-[230px_minmax(360px,1fr)_300px]">
        <aside className="border-b border-stone-300 bg-white/45 lg:border-b-0 lg:border-r">
          <div className="grid grid-cols-4 gap-px border-b border-stone-300 bg-stone-300 lg:grid-cols-2">{TACKLE_SYSTEMS.map(({ id, label, icon: Icon }) => <button type="button" key={id} aria-label={`选择${label}系统`} onClick={() => chooseSystem(id)} className={`min-h-16 bg-[#f3f1ed] px-2 py-3 text-[10px] ${system === id ? 'text-zinc-950 shadow-[inset_0_-2px_0_#18181b]' : 'text-zinc-500 hover:text-zinc-900'}`}><Icon size={15} className="mx-auto mb-1.5" aria-hidden="true" />{label}</button>)}</div>
          <div className="max-h-[420px] overflow-y-auto p-3"><small className="mb-2 block px-2 font-mono text-[9px] text-zinc-400">OBJECT INDEX / 器物索引</small>{visible.map((record) => <button type="button" key={record.id} onClick={() => setActiveId(record.id)} className={`w-full border-b border-stone-200 px-2 py-3 text-left text-xs leading-5 ${active?.id === record.id ? 'bg-white text-zinc-950' : 'text-zinc-600 hover:bg-white/60'}`}>{record.title}</button>)}</div>
        </aside>

        <div className="relative flex min-h-[390px] flex-col justify-between overflow-hidden bg-[#e8e5de] p-5 md:p-8">
          ...
          <svg viewBox="0 0 420 220" className="relative z-10 my-5 w-full" role="img" aria-label="钓具受力曲线示意">
            ...
            <motion.circle animate={{ cx: 400, cy: 34 + load * 0.75 }} transition={{ type: 'spring', damping: 20 }} r="8" fill="#f3f1ed" stroke="#27272a" strokeWidth="3" />
        ...
        <aside className="border-t border-stone-300 bg-white/65 p-6 lg:border-l lg:border-t-0 md:p-8">
          <AnimatePresence><motion.div key={`${active?.id}-${loadState.id}`} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8, position: 'absolute' }} className="space-y-6"><div><small className="font-mono text-[9px] text-zinc-400">LOAD READING / 负载读数</small><strong className="mt-2 block text-2xl font-semibold text-zinc-900">{loadState.label}</strong><p className="mt-3 text-sm leading-7 text-zinc-600">{loadState.description}</p></div>{active ? <div className="border-t border-stone-200 pt-5"><small className="font-mono text-[9px] text-zinc-400">SELECTED OBJECT / 当前器物</small><p className="mt-3 text-sm leading-7 text-zinc-700">{active.description}</p><button type="button" onClick={() => onOpen(active)} aria-label={`查看${active.title}详情`} className="mt-5 border-b border-zinc-800 pb-1 text-xs text-zinc-900">查看完整器物记录</button></div> : <p className="text-sm text-zinc-500">该系统尚无可用器物记录。</p>}</motion.div></AnimatePresence>
        </aside>
      </div>
      <p className="border-t border-stone-300 px-4 py-3 text-[9px] leading-5 text-zinc-500">本台呈现钓组中的相对受力变化，不替代产品参数、现场检查或制造商安全说明。</p>
    </section>
  )
}
