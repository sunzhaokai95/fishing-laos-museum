    ...
    <main className="min-h-screen bg-[#f5f5f7] text-zinc-800 px-4 md:px-8 py-14 md:py-20 relative overflow-hidden font-sans">
      ...
      <div className="max-w-7xl mx-auto relative z-10 space-y-10">
        ...
        <section className="space-y-7" aria-label="技法展台">
          <nav className="grid grid-cols-4 gap-px border border-zinc-200 bg-zinc-200 max-w-2xl" aria-label="技法类型">{TECHNIQUE_MODES.map(({ id, label }) => { const Icon = ICONS[id]; const selected = mode === id; return <button type="button" className={`relative min-h-16 bg-white px-2 flex flex-col sm:flex-row items-center justify-center gap-1.5 sm:gap-2 text-xs transition-colors ${selected ? 'text-zinc-950 shadow-[inset_0_-2px_0_#18181b]' : 'text-zinc-500 hover:text-zinc-900'}`} onClick={() => chooseMode(id)} key={id}><Icon size={16} aria-hidden="true" /><span>{label}</span></button> })}</nav>
